...
For production custody workflows, prefer hardware wallets or multisig signing flows. If a workflow needs offline signing or delayed submission at the protocol level, transactions that rely on a recent blockhash are constrained by the blockhash expiry window; use the official Solana [Durable Nonces](https://solana.com/docs/core/transactions/durable-nonces) guidance instead. Nonce transactions use System Program nonce instructions, including `AdvanceNonceAccount` as the first instruction, to support signing workflows that cannot complete within the recent blockhash window.
