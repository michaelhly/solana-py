...
def assert_encoded_array(buffer: bytearray, length: int, prev_length: int, expected: List[int]) -> bytearray:
...
def test_encode_length():
    ...
    expected = [0]
    buffer = assert_encoded_array(buffer, 0, prev_length, expected)
    prev_length += len(expected)

    expected = [5]
    buffer = assert_encoded_array(buffer, 5, prev_length, expected)
    prev_length += len(expected)

    expected = [0x7F]
    buffer = assert_encoded_array(buffer, 0x7F, prev_length, expected)
    prev_length += len(expected)

    expected = [0x80, 0x01]
    buffer = assert_encoded_array(buffer, 0x80, prev_length, expected)
    prev_length += len(expected)

    expected = [0xFF, 0x01]
    buffer = assert_encoded_array(buffer, 0xFF, prev_length, expected)
    prev_length += len(expected)

    expected = [0x80, 0x02]
    buffer = assert_encoded_array(buffer, 0x100, prev_length, expected)
    prev_length += len(expected)

    expected = [0xFF, 0xFF, 0x01]
    buffer = assert_encoded_array(buffer, 0x7FFF, prev_length, expected)
    prev_length += len(expected)
    ...
    buffer = assert_encoded_array(buffer, 0x200000, prev_length, expected)
    prev_length += len(expected)
