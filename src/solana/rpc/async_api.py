...
class AsyncClient(_ClientCore):  # pylint: disable=too-many-public-methods
    ...
    def __init__(
        ...
        self._provider = async_http.AsyncHTTPProvider(
            endpoint, timeout=timeout, extra_headers=extra_headers
        )
    ...
    async def get_signatures_for_address(
        ...
        body = self._get_signatures_for_address_body(
            account, before, until, limit, commitment
        )
    ...
    async def send_transaction(
        ...
        *signers: Keypair,
        ...
        if recent_blockhash is None:
            if self.blockhash_cache:
                try:
                    ...
                    recent_blockhash = self._process_blockhash_resp(
                        blockhash_resp, used_immediately=True
                    )
                    last_valid_block_height = (
                        blockhash_resp.value.last_valid_block_height
                    )
            else:
                blockhash_resp = await self.get_latest_blockhash(Finalized)
        ...
        txn_resp = await self.send_raw_transaction(
            txn.serialize(False), opts=opts_to_use
        )
        if self.blockhash_cache:
            blockhash_resp = await self.get_latest_blockhash(Finalized)
    ...
    async def validator_exit(self) -> ValidatorExitResp:
        ...
        """
        return await self._provider.make_request(
            self._validator_exit, ValidatorExitResp
        )
