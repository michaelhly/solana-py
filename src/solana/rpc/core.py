...
sys.path.append(os.path.abspath("../../../../solders"))
...
class _ClientCore:  # pylint: disable=too-few-public-methods
    ...
    def _get_balance_body(
        ...
        return GetBalance(
            pubkey.to_solders(), RpcContextConfig(commitment=commitment_to_use)
        )
    ...
    def _get_signatures_for_address_body(
        ...
        config = RpcSignaturesForAddressConfig(
            before=before, until=until, limit=limit, commitment=commitment_to_use
        )
    ...
    def _get_stake_activation_body(
        ...
        return GetStakeActivation(
            pubkey.to_solders(), RpcEpochConfig(epoch, commitment_to_use)
        )
    ...
    def _get_token_accounts_convert(
        ...
        if maybe_mint is not None:
            ...
            filter_to_use = RpcTokenAccountsFilterProgramId(
                maybe_program_id.to_solders()
            )
        else:
    ...
    def _simulate_transaction_body(
        ...
        config = RpcSimulateTransactionConfig(
            sig_verify=sig_verify, commitment=commitment_to_use
        )
    ...
    def _process_blockhash_resp(
        self, blockhash_resp: GetLatestBlockhashResp, used_immediately: bool
    ) -> Blockhash:
        ...
        if self.blockhash_cache:
            ...
            self.blockhash_cache.set(
                recent_blockhash, slot, used_immediately=used_immediately
            )
