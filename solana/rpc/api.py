...
class _ClientCore:  # pylint: disable=too-few-public-methods
    ...
    @staticmethod
    def _post_send(
        resp: types.RPCResponse, provider: Union[http.HTTPProvider, http.AsyncHTTPProvider]
    ) -> types.RPCResponse:
        if resp.get("error"):
...
class AsyncClient(_ClientCore):  # pylint: disable=too-many-public-methods
    ...
    async def __post_send_with_confirm(self, resp: types.RPCResponse, conf_comm: Commitment) -> types.RPCResponse:
        ...
        return await self.__confirm_transaction(resp["result"], conf_comm)

    async def __confirm_transaction(self, tx_sig: str, commitment: Optional[Commitment] = None) -> types.RPCResponse:
        # TODO: Use websockets and check confirmation with onSignature subscription.
        TIMEOUT = 30  # 30 seconds  pylint: disable=invalid-name
        elapsed_time = 0
        while elapsed_time < TIMEOUT:
